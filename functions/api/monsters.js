export async function onRequest(context) {
  ...
  if (request.method === 'GET') {
    ...
    if (code) {
      const monster = await env.DB.prepare('SELECT id, room, trainer, name, image_data, image_number, stats, history, created_at, battle_code FROM monsters WHERE battle_code = ? LIMIT 1').bind(code).first();
      if (!monster) return Response.json({error:'Monster not found'}, {status:404});
      return Response.json(monster);
    }
    if (id) {
      const monster = await env.DB.prepare('SELECT id, room, trainer, name, image_data, image_number, stats, history, created_at FROM monsters WHERE id = ?').bind(id).first();
      if (!monster) return Response.json({error:'Monster not found'}, {status:404});
      return Response.json(monster);
    }
    const result = await env.DB.prepare('SELECT id, room, trainer, name, image_data, image_number, stats, history, created_at FROM monsters ORDER BY created_at DESC').all();
    return Response.json(result.results || []);
  }
